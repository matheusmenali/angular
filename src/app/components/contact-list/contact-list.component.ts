...
export class ContactListComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject('contactService') private contactService) {
      ...
      this.personService.getPersons().subscribe( result => {
        ...
        const index = this.persons.findIndex( person => person.id == idShowingPerson);
        this.persons.splice(index, 1);
      })

      // carregando contatos
      // nao terminado
      this.contactService.getContacts(parseInt(this.route.snapshot.params.id_person)).subscribe( result => {
  ...
  onSubmit() {
    ...
    this.contactForm.value.person = parseInt(this.route.snapshot.params.id_person);
